...
class SDR{
     ...
     */
    public SDR (int mem){
        memoria = mem*1024; // Se convierten GB a mB
    ...
    private void calcularRAMDisp(){
        memoriaEnUso = 0;
        for (int i = 0; i<=(programasEnRAM.size()-1); i++){
    ...
    public void mostrarCola(){
        try {
            ...
            System.out.println("\nNo hay programas en la cola de espera\n");
        }
    ...
    public void nuevoCicloDeReloj(){
        ...
        for (int i = 0; i < colaDeEspera.size(); i++ ){
            if (colaDeEspera.get(i).getEspacio() <= memoriaRest){
